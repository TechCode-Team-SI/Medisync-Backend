...
export class Migration1722857654758 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`password_token\` DROP COLUMN \`userId\``,
    );
    await queryRunner.query(
      `ALTER TABLE \`confirm_email_token\` DROP COLUMN \`userId\``,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`confirm_email_token\` ADD \`userId\` varchar(36) NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE \`password_token\` ADD \`userId\` varchar(36) NULL`,
    );
    await queryRunner.query(
