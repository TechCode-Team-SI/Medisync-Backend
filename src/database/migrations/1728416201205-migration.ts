...
export class Migration1728416201205 implements MigrationInterface {
  name = 'Migration1728416201205';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`employee_profile\` ADD \`status\` tinyint NOT NULL DEFAULT 1`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`employee_profile\` DROP COLUMN \`status\``,
    );
    await queryRunner.query(
